		...
		<div className='bg-white w-full px-3 sm:px-6 mt-3 pb-6 rounded-t-md'>
			{items.length <= 0 ? (
				<div className='flex w-full flex-col gap-1 sm:gap-2 items-center justify-center py-6 sm:py-12'>
					<div className=' bg-gray-700/10 p-3 sm:p-4 rounded-full'>
						<HiShoppingBag className='text-xl sm:text-3xl text-gray-700' />
					</div>
				...
				<>
						...
						<ul className='py-3'>
							{items?.map((product: any) => (
								...
								>
									...
									<span className='w-1/6 text-sm sm:text-lg font-semibold text-gray-700'>
										<FormatMoney2 value={calculateSubtotal(product)} />
									</span>
									<span className='w-[8%]'>
										<BsTrash
											className='text-gray-700 text-lg sm:text-2xl cursor-pointer hover:scale-105 transition'
					...
					<div className='px-6 pt-5 pb-3 flex w-full justify-end '>
						<button
							...
							className='flex justify-center items-center py-2 sm:py-3 px-8 sm:px-14 text-sm sm:text-base border border-secondary-400 rounded-md hover:text-white hover:bg-gray-700 transition hover:border-transparent'
						>
							Empty cart
						</button>
					</div>
